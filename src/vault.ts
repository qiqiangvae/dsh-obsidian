...
export function resolveLayout(vaultPath: string, config: PluginConfig): VaultLayout {
...
export function isPortableFilename(name: string): boolean {
  ...
  if (PORTABLE_BAD_CHARS.test(name)) return false;
  if (PORTABLE_BAD_TAIL.test(name)) return false;
  return true;
}

/** Reject any path that resolves outside vaultPath. */
export function assertInsideVault(vaultPath: string, target: string): void {
...
export function completeFrontmatter(
  ...
  const merged: Frontmatter = { ...existing, ...patch };
  if (!merged.created) merged.created = existing.created ?? now;
  merged.updated = now;
  return merged;
}
...
export function upsertIndexEntry(
  ...
  if (m) {
    const body = m[2];
    if (body.includes(`[[${title}]]`)) return; // already present
...
// ──────────────────────────────────────────────────────────────────────────────
// Page write — type routing, frontmatter, index/log update, source delta
// ──────────────────────────────────────────────────────────────────────────────
...
export function collectUnresolvedLinks(body: string, knownTitles: Set<string>): string[] {
  ...
  for (const m of body.matchAll(linkRe)) {
    const t = m[1]!.trim();
    if (knownTitles.has(t) || seen.has(t)) continue;
    seen.add(t);
    out.push(t);
  }
  return out;
...
export function listAllTitles(wikiDir: string): Set<string> {
  const titles = new Set<string>();
  if (!existsSync(wikiDir)) return titles;
  for (const f of walkMd(wikiDir)) {
    const base = basename(f, extname(f));
    if (!isMachineryPage(base)) titles.add(base);
  }
  return titles;
}
...
export function writePage(
  ...
  appendLog(
    layout.logFile,
    `${args.type} [[${args.title}]]${args.source_path ? ` (source ${args.source_path})` : ''}`,
  );
...
export function* walkMd(root: string): Generator<string> {

...
import { walkMd, isMachineryPage, parseFrontmatter, resolveLayout, type PluginConfig } from './vault.js';
...
export function lint(vaultPath: string, config: PluginConfig, opts: { fix?: boolean } = {}): LintReport {
  ...
  for (const p of walkMd(layout.wikiDir)) {
    ...
    if (!fileByTitle.has(title)) fileByTitle.set(title, p);
  }
  ...
  for (const p of walkMd(layout.wikiDir)) {
    ...
    if (isMachineryPage(srcTitle) || MACHINERY.has(srcTitle)) continue;
  ...
  for (const title of titles) {
    if (MACHINERY.has(title) || isMachineryPage(title)) continue;
  ...
  const totals: LintReport['totals'] = { error: 0, warn: 0, info: 0 };
...
export function writeLintReport(vaultPath: string, config: PluginConfig, report: LintReport): string {
  const layout = resolveLayout(vaultPath, config);
  if (!existsSync(layout.metaDir)) mkdirSync(layout.metaDir, { recursive: true });
  ...
  for (const sev of ['error', 'warn', 'info'] as const) {
    ...
    for (const i of group) {
      ...
      if (i.suggestion) lines.push(`  - _suggestion:_ ${i.suggestion}`);
    }
    lines.push('');
  }
